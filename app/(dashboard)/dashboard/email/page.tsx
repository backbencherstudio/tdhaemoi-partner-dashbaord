...
export default function EmailPage() {
    ...
    const handleArchiveEmail = (id: number) => {
        ...
        // Mark email as archived (you can add an archived field to the email data)
    };
    ...
    return (
        <div className="flex flex-col md:flex-row h-full bg-gray-50">
            <div className="w-full md:w-64 bg-white border-b md:border-r border-gray-200 flex flex-col">
                ...
                <div className="p-4 border-b border-gray-200">
                    <button
                        ...
                        className="w-full border border-[#61A07B]  cursor-pointer text-[#61A07B] font-medium py-2 px-4 rounded-lg flex items-center gap-2 transition-colors hover:bg-[#61A07B] hover:text-white"
                    >
                        <Plus size={16} />
                        Verfassen
                    </button>
            ...
            <div className="flex-1 flex flex-col">
                {/* Show single email view or email list */}
                {selectedEmail ? (
                    <SingleEmailView
                        email={selectedEmail}
                            ...
                            {filteredEmails.length === 0 ? (
                                ...
                                <div className="divide-y divide-gray-200">
                                    {filteredEmails.map((email) => (
                                        <div
                                            key={email.id}
                                            className="p-3 sm:p-4 hover:bg-white hover:shadow-md transition-all duration-200 group relative cursor-pointer border-l-4 border-transparent hover:border-l-[#61A07B]"
                                            onClick={() => handleEmailClick(email)}
                                        >
                                            <div className="absolute top-3 sm:top-4 right-3 sm:right-4 flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity bg-white rounded-lg shadow-sm p-1">
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                            ...
                                            <div className="flex items-start gap-2 sm:gap-4">
                                                ...
                                                <button
                                                    ...
                                                    className="mt-1 hover:text-yellow-500 transition-colors p-1 rounded-full hover:bg-yellow-50"
                                                >
                                                    <Star
                                                        size={16}
            ...
            {showCompose && (
                <EmailManagement setShowCompose={setShowCompose} />
            )}
        </div>
    );
}
